...
router.post("/sync/push", async (req, res) => {
  ...
  if (!shopId || !deviceId || !operations) {
    return res.status(400).json({ ok: false, error: "shopId, deviceId and operations are required" });
  }
  if (operations.length > 200 || operations.some((op: any) => !validOperation(op))) {
    return res.status(400).json({ ok: false, error: "Invalid sync operation batch" });
  }
    ...
    if (operations.length) {
      await db.insert(syncOperations).values(
        operations.map((op: any) => ({
          ...
          payload: JSON.stringify(op.payload ?? null),
...
router.get("/sync/pull", async (req, res) => {
  ...
  if (!shopId || !Number.isFinite(since) || since < 0) {
    return res.status(400).json({ ok: false, error: "shopId and valid since are required" });
  }

  try {
    const rows = await db.select().from(syncOperations)
      .where(eq(syncOperations.shopId, shopId))
      .orderBy(asc(syncOperations.serverAt))
      .limit(limit);
    const operations = rows.filter(row => row.serverAt > since).map(row => ({
