...
export async function getNextInvoiceNumber(
  ...
  const row = await db.getFirstAsync<{ counter: number }>(
    'SELECT counter FROM invoice_counter WHERE id = 1'
  );
...
export async function exportDatabaseJSON(db: SQLite.SQLiteDatabase): Promise<string> {
  ...
  return JSON.stringify(
    { customers, products, sales, saleItems, khata, expenses, settings, counter, exportedAt: Date.now() },
    null,
    2
  );
}
...
export async function importDatabaseJSON(db: SQLite.SQLiteDatabase, json: string): Promise<void> {
  ...
  for (const c of data.customers ?? []) {
    await db.runAsync(
      ...
      [c.id, c.name, c.mobile, c.address, c.notes, c.credit_balance, c.created_at, c.updated_at]
    );
  }
  for (const p of data.products ?? []) {
    await db.runAsync(
      'INSERT OR REPLACE INTO products VALUES (?,?,?,?,?,?,?,?,?,?)',
      [p.id, p.name, p.barcode, p.purchase_price, p.selling_price, p.stock, p.low_stock_alert, p.unit, p.created_at, p.updated_at]
    );
  }
  for (const s of data.sales ?? []) {
    await db.runAsync(
      'INSERT OR REPLACE INTO sales VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
      [s.id, s.customer_id, s.customer_name, s.invoice_number, s.subtotal, s.discount, s.total, s.payment_method, s.cash_amount, s.upi_amount, s.credit_amount, s.notes, s.date, s.created_at]
    );
  }
  for (const si of data.saleItems ?? []) {
    await db.runAsync(
      'INSERT OR REPLACE INTO sale_items VALUES (?,?,?,?,?,?,?)',
      [si.id, si.sale_id, si.product_id, si.product_name, si.quantity, si.price, si.purchase_price]
    );
  }
  for (const k of data.khata ?? []) {
    await db.runAsync(
      'INSERT OR REPLACE INTO khata_transactions VALUES (?,?,?,?,?,?,?)',
      [k.id, k.customer_id, k.type, k.amount, k.note, k.date, k.created_at]
    );
  }
  for (const e of data.expenses ?? []) {
    await db.runAsync(
      'INSERT OR REPLACE INTO expenses VALUES (?,?,?,?,?,?)',
      [e.id, e.category, e.amount, e.note, e.date, e.created_at]
    );
  }
  for (const sv of data.settings ?? []) {
    await db.runAsync('INSERT OR REPLACE INTO settings VALUES (?,?)', [sv.key, sv.value]);
  }
  for (const ic of data.counter ?? []) {
    await db.runAsync('INSERT OR REPLACE INTO invoice_counter VALUES (?,?)', [ic.id, ic.counter]);
  }
}
