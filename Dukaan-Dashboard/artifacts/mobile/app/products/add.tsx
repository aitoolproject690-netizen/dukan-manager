...
import {
  ...
  ScrollView, Alert, Platform, KeyboardAvoidingView, Switch,
} from 'react-native';
...
export default function AddProductScreen() {
  ...
  const margin = purchasePrice && sellingPrice ? ((profit / (parseFloat(purchasePrice) || 1)) * 100).toFixed(1) : '0';

  const handleSave = async () => {
    if (!name.trim()) { Alert.alert('Name required'); return; }
    if (!sellingPrice || parseFloat(sellingPrice) <= 0) { Alert.alert('Enter a valid selling price'); return; }
    setSaving(true);
    try {
      const data = {
        name: name.trim(),
        ...
        <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: botPad + 20 }]} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              ...
              <TextInput style={[styles.input, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.card }]} value={purchasePrice} onChangeText={setPurchasePrice} placeholder="0" placeholderTextColor={colors.mutedForeground} keyboardType="numeric" />
            </View>
            <View style={{ width: 12 }} />
            <View style={{ flex: 1 }}>
              <Text style={[styles.label, { color: colors.mutedForeground }]}>Selling Price ({sym})</Text>
              <TextInput style={[styles.input, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.card }]} value={sellingPrice} onChangeText={setSellingPrice} placeholder="0" placeholderTextColor={colors.mutedForeground} keyboardType="numeric" />
            </View>
          </View>
          ...
          <View style={styles.row}>
            <View style={{ flex: 2 }}>
              ...
              <TextInput style={[styles.input, { color: colors.foreground, borderColor: colors.border, backgroundColor: colors.card }]} value={stock} onChangeText={setStock} placeholder="0" placeholderTextColor={colors.mutedForeground} keyboardType="numeric" />
            </View>
            <View style={{ width: 12 }} />
            <View style={{ flex: 1 }}>
